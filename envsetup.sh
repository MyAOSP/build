...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^baked_") ; then
       ...
       export BUILD_NUMBER=$((date +%s%N ; echo $BAKED_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       BAKED_BUILD=
    fi
    export BAKED_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${BAKED_DEVICES_ONLY}" != "z" ]; then
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var BAKED_VERSION)
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.baked.device=$BAKED_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $BAKED_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $BAKED_FIXUP_COMMON_OUT ]; then
